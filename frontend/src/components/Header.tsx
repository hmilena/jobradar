...
export default function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-md">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
          ...
          <Link href="/" className="flex items-center gap-2.5 group" onClick={() => setOpen(false)}>
            <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-brand-500 to-brand-700 shadow-sm group-hover:shadow-brand-300/50 group-hover:shadow-md transition-shadow">
              <svg
            ...
            <span className="font-semibold text-slate-900 tracking-tight">
              JobRadar
              <span className="ml-1.5 text-xs font-normal text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-full">
                PT
              </span>
            </span>
          </Link>
          ...
          <nav className="hidden sm:flex items-center gap-1">
            ...
            <a
              ...
              className="ml-2 btn-primary text-xs py-1.5 px-3 inline-flex"
            >
          ...
          <button
            className="sm:hidden flex flex-col items-center justify-center w-9 h-9 rounded-lg hover:bg-slate-100 transition-colors"
            onClick={() => setOpen((v) => !v)}
            aria-label={open ? "Fechar menu" : "Abrir menu"}
          >
            {open ? (
              /* X icon */
              <svg className="h-5 w-5 text-slate-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* Mobile menu overlay */}
      <div
        className={`fixed inset-0 z-40 sm:hidden transition-opacity duration-300 ${open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      ...
      {/* Mobile menu drawer */}
      <div
        className={`fixed top-0 right-0 z-50 h-full w-72 bg-white shadow-2xl sm:hidden flex flex-col transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "translate-x-full"}`}
      >
        {/* Drawer header */}
